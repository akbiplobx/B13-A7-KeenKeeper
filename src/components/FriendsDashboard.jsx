...
const FriendsDashboard = ({ friends }) => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-12 bg-[#f8fafc]">
      ...
      {/* Responsive grid for friend cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {friends.map((friend) => (
          
          /* Link to profile details */
          <Link to={`/friend/${friend.id}`} key={friend.id} className="block">
            <div className="bg-white rounded-[25px] p-8 flex flex-col items-center text-center transition-all cursor-pointer shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] border border-gray-50 h-full">
              <div className="mb-6">
                <img 
                  src={friend.picture} 
                  alt={friend.name} 
              ...
              <div className={`w-full max-w-[150px] py-2.5 rounded-full text-white text-[11px] font-extrabold uppercase tracking-widest shadow-md ${
                friend.status === 'overdue' ? 'bg-[#ff3b3b]' : 'bg-[#1a4a3e]'
              }`}>
